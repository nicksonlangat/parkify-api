...
from .response import CamelCaseResponse
...
def exception_handler(exc, context):
    ...
    By default we handle Django's built-in `Http404`,
    `PermissionDenied` and `ObjectDoesNotExist` exceptions.
    Also rest_framework's `APIException`'s are handled,
    since we use rest_framework for authentication.
    """
